...
# upload data
database = "https://raw.githubusercontent.com/kshiroky/datacon_hackathon/main/final_output.xlsx"
...
# violin plots
fig2, ax_ = plt.subplots(5, 3, figsize=(12, 11))
ax = []
for i in ax_:
    ax += i.tolist()
...
fig4, ax_ = plt.subplots(5, 3, figsize=(12, 11))
ax = []
for i in ax_:
    ax += i.tolist()
...
forest_cross_val_score = model_selection.cross_val_score(RandomForestRegressor(**forest_best_params),
                                                         x_train, y_train, cv=5)
forest_cross_val_accuracy = sum(forest_cross_val_score) / len(forest_cross_val_score)
forest_cross_val_mse = metrics.mean_squared_error(y_true=y_train, y_pred=forest_cross_val_pred)
print("RandomForestRegressor 5-fold cross-validation mean accuracy result: \n", forest_cross_val_accuracy)
